        ...
        sections: [
          {
            ...
            title: "Items",
            total: 100,
            lines: [{ id: "i", label: "Sofa", amount: 100, quantity: 1 }],
          },
          {
            key: "rules",
            title: "Applied Charges",
            total: 25,
            lines: [{ id: "r", label: "Trip Charge", amount: 25 }],
          },
        ],
      },
      status: "Sent",
      approvalTokenExpiresAt: new Date(),
    } as PublicEstimateApproval;
    const pdf = buildPublicEstimatePdf(estimate);
    expect(pdf.total).toBe("$125.00");
    expect(
      pdf.sections.find((section) => section.title === "Applied Charges")?.rows,
    ).toEqual([{ label: "Trip Charge", value: "$25.00" }]);
