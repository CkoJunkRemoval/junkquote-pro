...
export default function InvoiceManagement({initialStatus="All",initialPeriod}:{initialStatus?:InvoiceManagementStatus;initialPeriod?:InvoiceListPeriod}) {
  ...
  return <div className="contrast-controls mx-auto max-w-6xl p-6 sm:p-10">
    ...
    <div className="mt-7 flex flex-wrap gap-2">{invoiceManagementStatuses.map((item) => <button key={item} type="button" aria-pressed={status === item} onClick={() => { setStatus(item); setPeriod(undefined); setPage(1); }} className={`filter-pill rounded-full px-4 py-2 text-sm font-medium ${status === item ? "filter-pill--active" : ""}`}>{item}</button>)}</div>
