...
export default function InvoiceManagement() {
  ...
  useEffect(() => {
    let active = true;
    void listInvoicesAction({ status: status === "All" ? undefined : status, search, sort, page })
  ...
  return <div className="mx-auto max-w-6xl p-6 sm:p-10">
    ...
    <div className="mt-7 flex flex-wrap gap-2">{statuses.map((item) => <button key={item} type="button" onClick={() => { setStatus(item); setPage(1); }} className={`rounded-full px-4 py-2 text-sm font-medium ${status === item ? "bg-slate-900 text-white" : "bg-slate-200 text-slate-700"}`}>{item}</button>)}</div>
