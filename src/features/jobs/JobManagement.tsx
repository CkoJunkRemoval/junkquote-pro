...
import {jobManagementFilters,type JobManagementFilter,type JobManagementPeriod} from "./jobListFilters";

type Sort = "scheduled_asc" | "scheduled_desc" | "updated_desc" | "updated_asc";
type JobsResult = Awaited<ReturnType<typeof listJobsAction>>;

function labelStatus(status: string) { return status === "InProgress" ? "In Progress" : status === "Cancelled" ? "Canceled" : status; }

export default function JobManagement({initialFilter="All",initialPeriod}:{initialFilter?:JobManagementFilter;initialPeriod?:JobManagementPeriod}) {
  const [filter, setFilter] = useState<JobManagementFilter>(initialFilter);
  const [period, setPeriod] = useState<JobManagementPeriod>(initialPeriod);
  ...
  useEffect(() => {
    let active = true;
    void listJobsAction({ status: filter === "All" ? undefined : filter, period, search, sort, page, ...(crewId ? { crewId } : {}), ...(unassigned ? { unassigned: true } : {}) })
