...
import { NEW_ESTIMATE_HREF } from "@/components/navigation/DashboardQuickActions";
...
export default function EstimateComplete() {
  const { estimate, resetEstimate } = useEstimate();
  const router = useRouter();

  const estimatePackage = buildEstimatePackage(estimate);

  async function handleGeneratePdf() {
    const pdf = buildEstimatePdf(estimatePackage);
      ...
      <Card>
        <div className="space-y-8">
          <div className="text-center">
            ...
            <h1 className="mt-4 text-4xl font-bold">Estimate Complete</h1>
