...
vi.mock("@/lib/prisma", () => ({
  prisma: {
    company: {
      findUniqueOrThrow: mocks.companyFind,
...
vi.mock("@/lib/billing/stripe", async (importOriginal) => ({
  ...
  getStripeConnect: () => ({
    accounts: { create: mocks.accountCreate },
    accountLinks: { create: mocks.linkCreate },
  }),
}));
vi.mock("@/lib/payments/stripeConnect", () => ({
  retrieveAndSyncConnectedAccount: mocks.sync,
}));

import {
  refreshStripeConnectAction,
  returnFromStripeConnectAction,
...
describe("Stripe Connect management authorization and idempotency", () => {
  beforeEach(() => {
    ...
    mocks.companyFind.mockResolvedValue({
      id: "company-1",
      name: "Hauling",
      displayName: "Hauling",
      email: null,
  ...
  it("allows an Owner/Admin and creates one idempotent connected account", async () => {
    ...
    expect(mocks.accountCreate.mock.calls[0][1]).toEqual({
      idempotencyKey: "connect-company-company-1",
    });
  });
  ...
  it("returns a controlled error when connected-account creation is rejected", async () => {
    mocks.accountCreate.mockRejectedValue(
      new Stripe.errors.StripeInvalidRequestError({
        message: "Live Connect platform profile is incomplete.",
        type: "invalid_request_error",
      }),
    );
    await expect(startStripeConnectAction()).rejects.toThrow("NEXT_REDIRECT");
    expect(mocks.redirect).toHaveBeenCalledWith(
      "/settings/payments?connectError=1",
    );
    expect(mocks.companyUpdate).not.toHaveBeenCalled();
    expect(mocks.linkCreate).not.toHaveBeenCalled();
  });

  it("returns a controlled error when Account Link creation is rejected", async () => {
    mocks.companyFind.mockResolvedValue({
      id: "company-1",
      stripeConnectedAccountId: "acct_existing",
      stripeConnectDisconnectedAt: null,
    });
    mocks.linkCreate.mockRejectedValue(
      new Stripe.errors.StripeInvalidRequestError({
  ...
  it("retrieves authoritative account state on return", async () => {
    await expect(returnFromStripeConnectAction()).rejects.toThrow(
      "NEXT_REDIRECT",
    );
