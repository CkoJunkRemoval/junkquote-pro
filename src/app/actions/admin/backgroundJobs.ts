...
export async function retryBackgroundJobAction(jobId: string) { const c = await requireAdminTenant();if(!checkRateLimit(`job-retry:${c.companyId}:${c.user.id}`,ratePolicies.backgroundRetry).allowed)throw new AppError("RATE_LIMITED","Too many retry requests.");return databaseJobQueue.retry(c.companyId, jobId); }
