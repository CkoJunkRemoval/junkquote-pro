...
export async function downloadInvoicePdfAction(invoiceId: string) {
  const c = await requireOperationalTenant();if(!(await checkRateLimit(`pdf:${c.companyId}:${c.user.id}`,ratePolicies.pdf)).allowed)throw new AppError("RATE_LIMITED","Too many PDF requests.");
