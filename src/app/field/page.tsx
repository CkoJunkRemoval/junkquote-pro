...
import OfflineSyncCenter from "@/features/field/OfflineSyncCenter";
...
export default async function FieldPage() {
  ...
  const offlineJobs = [...new Map(
    [...data.today, ...data.upcoming, ...data.overdue, ...data.completedToday].map((job) => [
      job.id,
      {
        id: job.id,
  ...
  const page = <AppLayout><main className="space-y-5 p-4 pb-24 sm:p-6">
    ...
    {data.currentJob&&<div className="rounded-xl bg-blue-700 p-5 text-white"><p className="text-sm">Current job</p><Link className="inline-flex min-h-11 items-center text-xl font-semibold underline" href={`/field/jobs/${data.currentJob.id}`}>{data.currentJob.jobNumber??"Open job"} · {data.currentJob.fieldStage}</Link></div>}
    <OfflineSyncCenter companyId={tenant.companyId} userId={tenant.user.id} jobs={offlineJobs} />
