...
export async function GET(request: Request, { params }: { params: Promise<{ kind: string }> }) {
  ...
  let body = "";
  const includeNonCustomers = new URL(request.url).searchParams.get("includeNonCustomers") === "1";
  ...
  if (kind === "activation") body = csv(await getActivationFunnel(includeNonCustomers));
  if (kind === "usage") { const x = await getPlatformUsage(new Date(), includeNonCustomers); body = csv(x.daily); }
  if (kind === "subscriptions") { const x = await getPlatformSubscriptions(includeNonCustomers); body = csv([...x.statuses.map((r) => ({ dimension: "status", ...r })), ...x.plans.map((r) => ({ dimension: "plan", ...r }))]); }
  if (kind === "conversions") {
    const x = await getPlatformConversions(platformRange("all"), includeNonCustomers);
