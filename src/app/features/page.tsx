...
const featureEvidence = {
  "Quote the Job": [
    { image: productScreenshots.estimate, caption: "Quote the Job: itemized estimate review and complete pricing breakdown." },
    { image: productScreenshots.approval, caption: "Customer approval and delivery options for a completed estimate." },
  ],
  "Run the Job": [
    { image: productScreenshots.dispatch, caption: "Run the Job: scheduling, crew assignment, and dispatch status in one board." },
  ],
} as const;
...
export default function Features() {
  return <main className="min-h-screen overflow-x-hidden bg-[#050806] text-white">
    ...
    <div className="border-y border-white/10 bg-[#090d0a] px-5 py-16 sm:px-8"><div className="mx-auto max-w-5xl space-y-16">{groups.map(group => { const evidence = group.title in featureEvidence ? featureEvidence[group.title as keyof typeof featureEvidence] : []; return <section key={group.title} aria-labelledby={`feature-${group.title.toLowerCase().replaceAll(" ", "-")}`}><h2 id={`feature-${group.title.toLowerCase().replaceAll(" ", "-")}`} className="text-3xl font-black">{group.title}</h2><div className="mt-5 grid gap-5 md:grid-cols-2">{group.features.map(([name, copy]) => <article key={name} className="rounded-2xl border border-white/10 bg-[#0d130e] p-6"><h3 className="text-xl font-bold">{name}</h3><p className="mt-3 leading-relaxed text-slate-300">{copy}</p></article>)}</div>{evidence.length > 0 && <div className="mt-8 space-y-8">{evidence.map(item => <ProductScreenshot key={item.image.src} image={item.image} caption={item.caption} />)}</div>}</section>; })}</div></div>
