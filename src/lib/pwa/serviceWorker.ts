...
const FIELD_OFFLINE_URL="/offline/field";
...
self.addEventListener("install",event=>event.waitUntil(caches.open(CACHE_NAME).then(async cache=>{for(const path of [OFFLINE_URL,FIELD_OFFLINE_URL]){const response=await fetch(path,{cache:"no-store"});if(!cacheable(response))continue;const html=await response.clone().text();await cache.put(path,response);const assets=[...html.matchAll(/(?:src|href)="(\\/_next\\/static\\/[^"]+)"/g)].map(match=>match[1]);for(const asset of new Set(assets)){const assetResponse=await fetch(asset,{cache:"no-store"});if(cacheable(assetResponse))await cache.put(asset,assetResponse);}}})));
...
self.addEventListener("fetch",event=>{const request=event.request;if(request.method!=="GET")return;const url=new URL(request.url);if(url.origin!==self.location.origin)return;if(request.mode==="navigate"){const fieldNavigation=url.pathname.startsWith("/field")||url.pathname===FIELD_OFFLINE_URL;event.respondWith(fetch(request).catch(()=>fieldNavigation&&url.pathname!==FIELD_OFFLINE_URL?Response.redirect(FIELD_OFFLINE_URL,302):caches.match(fieldNavigation?FIELD_OFFLINE_URL:OFFLINE_URL)));return;}if(sensitivePath(url.pathname)||request.headers.has("authorization"))return;if(!safePath(url.pathname))return;event.respondWith((async()=>{const cached=await caches.match(request);if(cached)return cached;const response=await fetch(request);if(cacheable(response)){const cache=await caches.open(CACHE_NAME);await cache.put(request,response.clone());}return response;})());});
