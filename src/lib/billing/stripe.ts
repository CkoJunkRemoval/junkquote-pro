...
import { inspectStripeConfiguration, stripeConfigurationDiagnostic } from "./stripeConfiguration";
...
export function isBillingAvailable(env: NodeJS.ProcessEnv = process.env) { return inspectStripeConfiguration(env).available; }
export function assertBillingAvailable() { const status = inspectStripeConfiguration(); if (!status.available) { console.error("[billing] Stripe configuration unavailable", stripeConfigurationDiagnostic(status)); throw new BillingUnavailableError(); } }
