...
import type { CompanySubscription, MembershipRole, SubscriptionPlan } from "@/generated/prisma/client";
...
const owner = (role?: MembershipRole) => role === "Owner";
export async function canAccessFeature(companyId: string, feature: BillingFeature, role?: MembershipRole) { if (owner(role)) return true; const e = await getCompanyEntitlements(companyId); return e.config.features.includes(feature); }
export async function requireSubscriptionAccess(companyId: string) { return getCompanyEntitlements(companyId); }
export async function requireFeature(companyId: string, feature: BillingFeature, role?: MembershipRole) { const e = await getCompanyEntitlements(companyId); if (!owner(role) && !e.config.features.includes(feature)) throw new BillingAccessError(`Upgrade from ${e.config.name} to use this feature.`, "FEATURE_UNAVAILABLE"); return e; }
...
export async function canCreateEstimate(companyId: string, now = new Date(), role?: MembershipRole) { if (owner(role)) return true; const e = await getCompanyEntitlements(companyId, now); if (e.config.monthlyEstimateLimit === Number.MAX_SAFE_INTEGER) return true; const usage = await getEstimateUsage(companyId, now); if (usage.used >= usage.limit) throw new BillingAccessError(`The ${e.config.name} plan includes ${usage.limit} estimates per UTC calendar month. Upgrade to create another estimate.`, "PLAN_LIMIT"); return true; }
export async function canInviteUser(companyId: string, role?: MembershipRole) { if (owner(role)) return true; const e = await getCompanyEntitlements(companyId); const count = await prisma.companyMembership.count({ where: { companyId, status: "Active" } }); if (count >= e.config.userLimit) throw new BillingAccessError(`The ${e.config.name} plan allows ${e.config.userLimit} active user${e.config.userLimit === 1 ? "" : "s"}.`, "PLAN_LIMIT"); return true; }
export async function requireCrewCapacity(companyId: string, role?: MembershipRole) { if (owner(role)) return; const e = await requireFeature(companyId, "operations", role); const count = await prisma.crew.count({ where: { companyId, active: true } }); if (count >= e.config.crewLimit) throw new BillingAccessError("Your plan's crew limit has been reached.", "PLAN_LIMIT"); }
export async function requireTruckCapacity(companyId: string, role?: MembershipRole) { if (owner(role)) return; const e = await requireFeature(companyId, "operations", role); const count = await prisma.fleetAsset.count({ where: { companyId, type: "Truck", status: { not: "Retired" } } }); if (count >= e.config.truckLimit) throw new BillingAccessError("Your plan's truck limit has been reached.", "PLAN_LIMIT"); }
