...
export async function respondToEstimateApproval(
  token: string,
  ...
  if (!["Sent", "Viewed"].includes(estimate!.status))
    throw new Error("This estimate has already been finalized.");
  const next = response === "approve" ? "Approved" : "Declined";
  if (response === "approve")
    validateSignature(signerName ?? "", signatureData ?? "");
  await transitionEstimate(estimate!.companyId, estimate!.id, next, {
