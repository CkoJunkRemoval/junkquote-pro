...
import {recordEstimateEvent} from "@/lib/estimates/estimateEvents";
export async function updateInvoiceStatus(companyId:string,invoiceId:string,nextStatus:InvoiceWorkflowStatus) {
  ...
  else if(nextStatus==="Sent")await recordEstimateEvent({companyId,estimateId:invoice.estimateId,eventType:"Invoice Sent",category:"Invoice",actor:{type:"Employee",displayName:"Team member"},summary:"Team member sent invoice",visibility:"Both",metadata:{invoiceId},attachments:[{referenceType:"Invoice",referenceId:invoiceId,displayName:"Invoice"}]});
