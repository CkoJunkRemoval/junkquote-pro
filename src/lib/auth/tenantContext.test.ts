...
import { AuthorizationError, requireAdminTenant, requireOperationalTenant, requireTenantContext, requireTenantRole } from "./tenant";
...
describe("typed tenant context", () => {
  ...
  it("keeps authenticated dashboard role denial controlled", async () => { mocks.auth.mockResolvedValue({ user: { id: "user-a" } }); mocks.findMembership.mockResolvedValue({ ...membership, role: "Crew" }); await expect(requireTenantRole("Owner", "Admin", "Manager", "Office")).rejects.toMatchObject({ name: "AuthorizationError", code: "FORBIDDEN" }); });
  it("lets Owner bypass normal tenant role lists without granting that bypass to other roles", async () => { mocks.auth.mockResolvedValue({ user: { id: "user-a" } }); mocks.findMembership.mockResolvedValue(membership); await expect(requireTenantRole("Crew")).resolves.toMatchObject({ role: "Owner" }); mocks.findMembership.mockResolvedValue({ ...membership, role: "Admin" }); await expect(requireTenantRole("Crew")).rejects.toMatchObject({ code: "FORBIDDEN" }); });
});
