...
export async function getCompanyBranding(companyId: string) {
  const company = await prisma.company.findUnique({ where: { id: companyId }, select: { id: true, name: true, legalName: true, displayName: true, email: true, phone: true, website: true, addressLine1: true, addressLine2: true, city: true, state: true, postalCode: true, logoUrl: true, primaryColor: true, secondaryColor: true, invoicePrefix: true, estimatePrefix: true, defaultTaxRate: true, defaultPaymentTermsDays: true, defaultEstimateExpirationDays: true, defaultMinimumCharge: true, timezone: true, currencyCode: true, createdAt: true, updatedAt: true } });
  if (!company) throw new Error("Company not found.");
  return company;
}

export async function updateCompanyBranding(companyId: string, input: CompanySettingsInput) {
  ...
  return prisma.company.update({ where: { id: companyId }, data, select: { id: true, name: true, legalName: true, displayName: true, email: true, phone: true, website: true, addressLine1: true, addressLine2: true, city: true, state: true, postalCode: true, logoUrl: true, primaryColor: true, secondaryColor: true, invoicePrefix: true, estimatePrefix: true, defaultTaxRate: true, defaultPaymentTermsDays: true, defaultEstimateExpirationDays: true, defaultMinimumCharge: true, timezone: true, currencyCode: true, createdAt: true, updatedAt: true } });
}
...
export async function uploadCompanyLogo(companyId: string, file: File) {
  ...
  try { const updated = await prisma.company.update({ where: { id: company.id }, data: { logoUrl } }); await localCompanyLogoStorage.remove(companyId, company.logoUrl); return updated; } catch (error) { await localCompanyLogoStorage.remove(companyId, logoUrl); throw error; }
}
