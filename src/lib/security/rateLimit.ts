import {
  coordinationStore,
  secureCoordinationKey,
} from "@/lib/distributed/store";
...
export async function checkRateLimit(
  ...
  try {
    const result = await coordinationStore().incrementWindow(
      secureCoordinationKey("rate", key),
      policy.windowMs,
    );
    ...
    logger.error("security.redis_rate_limit_failure", {
      category: key.split(":", 1)[0],
    });
...
export function clearRateLimits() {
  /* compatibility: distributed counters intentionally cannot be globally cleared */
}
export const ratePolicies = {
  ...
  publicEstimate: { limit: 60, windowMs: 15 * 60_000 },
  billing: { limit: 30, windowMs: 60_000 },
  systemAdmin: { limit: 30, windowMs: 60_000 },
  ...
  fieldUpload: { limit: 60, windowMs: 60_000 },
  fieldChangeOrder: { limit: 20, windowMs: 60_000 },
} as const;
