...
export default function GlobalSearch({ dashboard = false }: { dashboard?: boolean }) {
  ...
  const field = <div className="relative w-full"><Search aria-hidden="true" className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input ref={input} type="search" role="combobox" aria-autocomplete="list" aria-label="Search company workspace" aria-controls="global-search-results" aria-expanded={open} value={query} onFocus={() => setOpen(true)} onChange={(event) => { setQuery(event.target.value); setOpen(true); }} placeholder="Search customers, properties, estimates, jobs…" className="header-search min-h-11 w-full rounded-lg py-2.5 pl-10 pr-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400" /></div>;
