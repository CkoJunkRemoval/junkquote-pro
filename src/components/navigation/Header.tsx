...
export default function Header({ onMenu }: { onMenu: () => void }) {
  ...
  return <header className="flex h-20 items-center gap-4 border-b border-slate-200 bg-white px-4 sm:px-6">
    ...
    <div className="ml-auto flex items-center gap-4"><button className="rounded-lg p-2 text-slate-500 hover:bg-slate-100"><Bell size={20} /></button><div className="flex items-center gap-2 text-sm">{company?.logoUrl ? <Image src={company.logoUrl} alt="Company logo" width={32} height={32} className="h-8 w-8 rounded object-contain" /> : <UserCircle2 size={30} />}<span className="hidden sm:block"><strong className="block">JunkQuote Pro</strong><span className="text-xs text-slate-500">{company?.displayName ?? "Your company"}</span></span></div><button onClick={() => void signOut({ callbackUrl: "/sign-in" })} className="text-sm text-slate-600 hover:underline">Sign out</button></div>
  </header>;
}
