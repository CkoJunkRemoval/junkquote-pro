...
import { recordPayment, refundPayment } from "@/lib/payments/paymentMutations";
...
describe("real database tenant isolation", () => {
  ...
  it("isolates invoices, payment recording, and refunds", async () => { const { a, b } = await createTenantFixtures(); expect(await getInvoiceDetail(a.company.id, b.invoice.id)).toBeNull(); await expect(recordPayment(a.company.id, b.invoice.id, { amount: 5, method: "Cash", paymentDate: new Date() })).rejects.toThrow("Invoice not found"); await expect(refundPayment(a.company.id, b.payment.id)).rejects.toThrow("Payment not found"); });
