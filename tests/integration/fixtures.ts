...
async function createTenant(label: "A" | "B", invoiceNumber: number) {
  ...
  const user = await prisma.user.create({
    data: {
      companyId: company.id,
      email: `owner-${label.toLowerCase()}@test.invalid`,
  ...
  const officeUser = await prisma.user.create({
    data: {
      companyId: company.id,
      email: `office-${label.toLowerCase()}@test.invalid`,
      passwordHash: "integration-only",
      role: "OFFICE",
    },
  });
  const officeMembership = await prisma.companyMembership.create({
    data: { userId: officeUser.id, companyId: company.id, role: "Office" },
  });
  const crewUser = await prisma.user.create({
    data: {
      companyId: company.id,
      email: `crew-${label.toLowerCase()}@test.invalid`,
      passwordHash: "integration-only",
      role: "CREW_MEMBER",
    },
  });
  const crewMembership = await prisma.companyMembership.create({
    data: { userId: crewUser.id, companyId: company.id, role: "Crew" },
  });
  const suspendedUser = await prisma.user.create({
    data: {
      companyId: company.id,
      email: `suspended-${label.toLowerCase()}@test.invalid`,
      passwordHash: "integration-only",
      role: "OFFICE",
    },
  });
  const suspendedMembership = await prisma.companyMembership.create({
    data: {
  ...
  const crew = await prisma.crew.create({
    data: { companyId: company.id, name: `Crew ${label}` },
  });
  const photo = await prisma.jobPhoto.create({
    data: {
      ...
      jobId: job.id,
      category: "Before",
      fileUrl: `/api/private/assets/job-photos/${company.id}/${job.id}/photo.jpg`,
      fileName: "photo.jpg",
      mimeType: "image/jpeg",
      fileSize: 10,
      sortOrder: 0,
...
export async function createTenantFixtures() {
  return {
    a: await createTenant("A", 91001),
    b: await createTenant("B", 92001),
  };
}
